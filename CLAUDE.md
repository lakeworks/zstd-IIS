...
**Breaking change vs. upstream**: the upstream `kimboslice99/zstd-IIS` README lists `120`, `121`, `122` as valid "slowest" values — this fork rejects them (they fall in the `118`–`122` hard-fail band noted above). The failure is loud: `Compress` returns `E_INVALIDARG`, the scheme stops compressing, and there is no graceful fallback to identity — IIS surfaces the error to the response pipeline, so you see it on the first request. Audit `applicationHost.config` per the "Invalid IIS-config bands" note above before deploying.
